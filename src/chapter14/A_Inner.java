...
// 클래스 간의 관계를 명확하게 표현
// 내부 클래스는 외부 클래스의 맴버(필드, 메소드)에 접근 가능

// 종류
/*
 	1. (비정적) 내부 클래스
 		: 외부 클래스의 인스턴스에 속함
 		>> 외부 클래스의 인스턴스 변수 및 메소드에 접근 가능
 	2. 정적(static) 내부 클래스
 		: 외부 클래스의 정적 멤버에 속함
 		>> 외부 클래스의 인스턴스 변수에 접근 불가
 		>> 외부 클래스의 정적 멤버에만 접근가능
 		>> 외부 클래스의 인스턴스 생성 없이 인스턴스화 가능
 	3. 메소드 내부 클래스(지역 클래스)
 		: 메소드 내에서 정의된 내부 클래스
 		>> 해당 메소드 내에서만 사용 가능
 		>> 메소드의 지역 변수에만 접근 가능
 		>> 로컬 변수(외부 클래스 멤버 변수)가 final인 경우에만 접근가능
 	4. 익명(내부) 클래스
 		: 이름이 없는 내부 클래스
...
class OuterClass{
	...
	// 3. 메소드 내부 클래스(지역 클래스)
	void outerMethod() {
		String localVar = "메소드 로컬 변수1";
		//localVar = "메소드 로컬 변수 값 변경"; - 지역 변수는 암묵적으로 final로 간주합니다(생략 가능)
		final String localFinalVar = "메소드 로컬 변수(Final)";
		//localFinalVar = "메소드 로컬 변수 값 변경";		
		class MethodClass{
			void display() {
		...
		// 로컬 클래스의 인스턴스화
		// 메소드 내부에서만 쓰이기 때문에 인스턴스화도 내부에서 일어남
		MethodClass methodClass = new MethodClass();
		methodClass.display();
	}
}
// 추상 클래스: 하나 이상의 추상 메소드 포함
abstract class AbstractClass{
	// 추상 클래스는 구현 클래스를 가질 수 있음
	// >> 추상 메소드에 abstract 키워드 생략 불가
	abstract void dispaly();// 추상 메소드: 구현부 {} 가 없는 메소드
}
// 인터페이스
interface InterfaceClass{
	// 필드는 public static final 생략
	// 인터페이스는 일반 구현 클래스를 가질 수 없음
	// >> static 메소드와, default 메소드는 가능(예외)
...
public class A_Inner {
	public static void main(String[] args) {
		...
		System.out.println("메소드 내부 클래스(지역 클래스)");
		OuterClass outerClass2 = new OuterClass();
		outerClass2.outerMethod();
		
		System.out.println("익명 내부 클래스");
		// 클래스타입 변수명  = new 클래스명(){메소드수현-추상 메소드/인터페이스의 추상 메소드}
