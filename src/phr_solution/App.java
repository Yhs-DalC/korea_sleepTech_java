 ...
 * 
 * 개인건강기록 솔루션을 위한 간단한 콘솔 기반 응용 프로그램
 * >> 환자의 기본 정보를 관리, 환자의 건강 기록 등록, 수정, 조회
 * >> 특정 조건에 맞는 건강 기록을 필터링하고 집계 기능
 * 
 * 기능 & 로직
 * 환자 정보 관리 >> CR(단건/전체)UD
 * 건강 기록 관리 >> CRUD + 필터링(조회)
 * 
 * 프로그램 구조
...
public class App {
	...
	private static void displayMenu() {
		System.out.println("\n[메뉴 선택]");
		// 이스케이프 문자: 문자 제어 코드, 문자열 내에서 특수한 기능을 수행하는 문자
		// \n 줄바꿈, \\(\ 기호), \"(" (기호 등등
		System.out.println("1. 환자 등록");
		System.out.println("2. 환자 정보 전체 조회");//error
		System.out.println("3. 환자 정보 단건 조회");
		System.out.println("4. 환자 정보 수정");//error
	...
	private static int getChoice() {
		while(!sc.hasNextInt()) {// 입력값이 숫자면 true 아니면 false 반환
	...
	private static PatientRequestDto createPatientRequest() {
		...
		try {
			...
			String gender = getInput("환자 성별을 입력하세요");
	...
	private static RecordRequestDto createRecorRequest() {
		...
		try {
			...
			String traetment = getInput("처방 내용을 입력하세요");
			
			dto = new RecordRequestDto(patientId, dateOfVisit, diagnosis, traetment);
		} catch(Exception e) {
			System.out.println(e.getMessage());
		}
		return dto;
	...
	private static boolean processChoice(int choice) {
		...
		case 6:{
			RecordRequestDto requestDto = createRecorRequest();
		...
		case 9:{
			long id = getIdInput();
			healthRecordController.deletRecord(id);
			break;
		}
		case 10:{
