...
// >> 모듈의 독립성을 결함도(Coupling)와 응집도(Cohesion)의 기준 단계로 측정

// 1. 결합도(낮은 결합도)
// 두 개의 클래스나 모듈이 얼마나 강하게 연결되어 있는지 나타냄
// 모듈과 모듈간의 의존정도를 낮춰야 한다
// 낮은 결합도: 두 모듈이 서로 독립적으로 동작
// 높은 결합도: 두 모듈이 서로 얽혀있는 관계
// >> 낮은 결합도 : 프로그램을 유지보수하기 쉽고, 새로운 기능을 추가하거나 기존 코드를 수정할 때 오류 가능성을 낮춤

// 2. 응집도(높은 응집도)
// 하나의 클래스나 모듈 안에서 관련된 작업들끼리 얼마나 잘 모여 있는지를 나타냄
// 한 모듈 내의 구성요소들 간의 연관 정도
// 높은 응집도: 하나의 클래스가 관련된 일들만 하고, 해당 일이 잘 명시됨
// 낮은 응집도: 하나의 클래스가 너무 많은 일을 하거나, 관련 없는 일을 하는것
// >> 높은 응집도 : 프로그램에 대한 이해, 유지보수를 용이하게 만듬
...
class KoreanTeacher implements Teacher{
	@Override
	public void teach() {
		System.out.println("국어 선생님은 국어를 가르친다");
	}
}
class MathTeacher implements Teacher{
	@Override
	public void teach() {
		System.out.println("수학 선생님은 수학을 가르친다");
	}
}
