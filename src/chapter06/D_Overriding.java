...
// 오버라이딩(Overriding)
// 자식 클래스가 부모 클래스에서 상속받은 '메소드'를 자신의 상황에 맞게 '재정의'
// 상속 관계에 있는 클래스들 사이에서 사용
// 부모 클래스의 기본 동작을 자식 클래스에서 변경하거나 확장할때 사용
...
//	}
	...
	// 부모 클래스의 메소드를 자식 클래스에서 재정의했다는것을 명시적으로 나타냄
	// @Override를 생략해도 오류가 나지 않는 이유
	// 자바 컴파일러가 메소드 시그니처(이름, 매개변수, 반환타입)을 비교하여 부모 클래스의 메소드를 자식 클래스에서 재정의했는지 판단
	
	// 상속받은 메소드명과 동일한 메소드를 자식 클래스에서 정의하는 경우
	// >>자동으로 재정의임을 판단
	@Override
	void exercise() {
...
public class D_Overriding {
	public static void main(String[] args) {
		...
		// 오버로딩(Overloading)과 오버라이드(Overriding)
		// 공통점: 메소드의 이름을 재사용하는 방법(다형성 - 같은 이름으로 다른 행위 처리)
		
		// 오버로딩: 같은 클래스 내에서 같은 이름을 가진 메소드를 여러개 정의
		// 메소드명은 동일. 매개변수의 타입이나 개수, 순서 중 적어도 하나 이상은 달라야 함
		// 반환타입 상관 X
		// 메소드 동작을 다양한 상황에 맞게 조정
		
		// 오버라이딩: 상속관계에 있는 클래스에서 사용
		// 메소드 시그니처(반환타입, 이름, 매개변수)가 동일
		// 메소드의 동작을 변경하거나 확장
