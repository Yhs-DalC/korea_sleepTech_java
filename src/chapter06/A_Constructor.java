...
// 생성자 조건
// 클래스명과 동일한 이름을 사용, 리턴값이 없는 메소드 형식(void 타입이 생략)

// 컴파일러가 제공하는 기본 생성자
// 클래스에 생성자가 없으면 자동 생성되는 빈 생성자
// 형태는 매개변수가 없으며 아무런 작업도 하지 않음(구현부 내의 코드가 없음)
...
// this 키워드
// this: 인스턴스 자신을 참조 >> 모든 인스턴스 메소드와 생성자 내부에서 사용 가능
// this(): 같은 클래스 내의 다른 생성자 호출>> 중복 코드 제거와 유지보수성 향상에 기여


public class A_Constructor {
	public static void main(String[] args) {
		Example1 example1 =  new Example1();// 빈 생성자 호출 >> 전달할 인자 없음
		Example2 example2 =  new Example2();// 빈 생성자 호출
		...
		Car car2 = new Car("white", "menual", 2);// 사용자 정의 생성자를 호출
		// 인스턴스의 생성: 생성자가 인스턴스 생성을 하는것은 아니다
		// new 연산자가 인스턴스를 생성하는 것
	}
}
