...
// 자바에서 배열과 리스트의 정렬
//1. 배열(arrays 클래스) 정렬: Arrays 클래스의 정적 메소드(.sort()) 사용
//>> 오름차순 정렬만 지원
//## 내림차순 정렬 - Comparator 클래스를 사용 .reverseOrder() 정적 메소드 사용

//2. 리스트 정렬: Collections.sort() >> List 인터페이스의 sort()메소드 사용
...
public class A_Sort {
	public static void main(String[] args) {
		...
		// >> Comparator<T>을 반환하는 메소드: 객체(타입)간의 비교(기본데이터타입 비교 X)
		Integer[] nums = {5, 3, 6, 2, 7};
		Arrays.sort(nums, Collections.reverseOrder());
		
		System.out.println(nums[0]);// 7
		System.out.println(nums[4]);// 3
		
		// 2. 리스트 정렬
		// 기본 정렬: sort() 오름차순, Collections.sort(데이터, Collections.reverseOrder()); - 내림차순
		...
		//## Comparator: 두 객체를 비교해서 정렬 순서를 결정
		
		
		// 3. 스트림 API 사용 정렬
		// 정렬된 결과를 새로운 리스트로 반환
		List<Integer> listNumbers = new ArrayList<Integer>(List.of(8, 5, 2, 4, 1));
		...
		// 두 요소를 비교한 결과를 반환
		//>> 반환값이 양수: b가 a보다 앞으로 옴
		//>> 반환값이 0: 순서 변경 x(두 수가 같음)
		//>> 반환값이 음수: a가 b보다 앞에 옴
		
		// 오름차순 (a, b) -> a - b
		
		List<String> characterNames = new ArrayList<String>(List.of("Tom", "Jerry", "Pororo", "Loopy"));
		// 캐릭터 이름의 길이를 오름차순으로 정렬
		List<String> sortedCharator = characterNames.stream()
				.sorted((n1, n2) -> n1.length() - n2.length())
				.collect(Collectors.toList());
		
		System.out.println(sortedCharator);
		
		// 불변성 데이터에 대한 정렬 >> 스트림API
		// 원본 데이터 그 자체에 대한 정렬 >> Comparator 클래스의 .comparing 메소드 사용
	}
}
