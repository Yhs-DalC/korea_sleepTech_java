...
public class UserServiceImpl implements UserService{
	...
	public void registerUser(String userId, String password, String name, String email) {
		...
		//optionalUser.get(): optional 내부의 데이터를 꺼내오는 메소드
		//>> 데이터가 존재하지 않아도 (null) 반환
		//>> 값이 존재하지 않을 때 NoSuchElementCxception 예외 발생
	...
	public void login(String userId, String password) {
		// ifPresentOrElse(a, b)
		// 정보가 존재하는 경우 a로직, 존재하지 않을 경우 b로직
		userRepository.findByUserId(userId).ifPresentOrElse(user -> {
			//userId에 일치하는 정보는 확인, 해당 유저의 비밀번호 일치여부 확인
