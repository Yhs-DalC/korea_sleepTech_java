...
//## 기본 데이터 타입
// byte, short, int, long, float, double, char, boolean, void(메소드 반환)
// >> 각 데이터 타입에 해당되는 Wrapper 클래스가 존재

// java.lang 패키지에 정의
// 컬렉션 프레임워크에서는 내부에 객체만을 저장
// >> 기본 데이터 타입의 저장을 Wrapper 클래스로 변환하여 저장

// Byte, Short, Integer, Long, Float, Double, Charator, Boolean, Void

// Boxing(박싱) & UnBoxing(언박싱)
// 기본 타입과 해당 타입의 Wrapper 클래스 간의 자동 변환
// 자바 컴파일러가 자동 처리

// 박싱: 기본 >> Wrapper 클래스의 객체로 변환하는 과정

// 언박싱: Wrapper >> 기본 타입으로 변환하는 과정

// 장단점
// 장점: 기본 타입과 객체 타입 간의 호환이 용이
// 단점: 객체 생성 과정에서 메모리 사용량 증가
//		null값이 할당될 수 있는 Wrapper 객체를 언박싱할 경우 NullPointerException 발생 위험
...
public class A_Wrapper {
	public static void main(String[] args) {
		...
		// Wrapper 클래스의 내부 메소드
		// ## Wrapper 클래스 내부의 toString() 메소드는 객체가 보유하고 있는 실제 값이 출력
		
		// Integer
		// 생성자: Integer(int value)
		Integer myInteger = new Integer(50);
		new Integer("50");// 문자열을 받아 해당하는 int값을 갖는 Integer 객체 생성
		//new Integer("안녕");
		...
		// parseInt(): 문자열을 기본 int 타입으로 파싱
		// String >> Integer >> int
		int parseInt = Integer.parseInt("123");
		System.out.println(parseInt);
		
		//compareTo(Integer anotherInt): 두 Integer 객체를 비교해서 같으면 0, 호출 객체(a)가 더 크면 양수, 더 작으면 음수
