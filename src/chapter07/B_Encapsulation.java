...
// 캡슐화(Encapsulation)
// OOP에서 데이터와 해당 데이터를 처리하는 메소드(함수)를 하나로 묶는 것을 의미
// 객체 내부의 구현을 숨기고, 외부에서는 제공된 메소드를 통해서만 접근하도록 설정
...
// 데이터 보호/유지보수성 향상을 목적으로 데이터 필드(속성)를 private 선언
// 데이터에 접근하거나 수정할 때 getter와 setter 메소드를 사용하는 체계

// getter: 객체의 특정 필드 값을 읽는 메소드
// get<필드명>(); >> getName, getAge ...

// setter: 객체의 특정 필드 값을 설정하는 메소드
// set<필드명>(); >> setName, setAge ...
