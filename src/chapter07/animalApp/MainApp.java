...
public class MainApp {
	public static void main(String[] args) {
		// 서비스 계층(실행) 생성
		// service 객체의 handleAnimal 메소드는 Animal 타입의 객체를 인자로 받음
		...
		if (cat instanceof Cat) {
			Cat onlycat = (Cat) cat;// 다운캐스팅: 해당 클래스가 가진 고유 멤버(필드, 메소드)에 접근 가능
			onlycat.eat();
		}
		
		if (dog instanceof Cat) {// false 조건문 안의 오류. 구문이 실행되지 않는다
			Cat dogCat = (Cat) dog;
			dogCat.eat();
		}
